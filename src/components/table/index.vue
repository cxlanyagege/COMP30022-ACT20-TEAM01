<template>
  <div>
    ...
    <el-dialog v-if="showProfileDialog" title="Student Profile" :visible.sync="showProfileDialog">
      <student-profile :student-info="selectedStudentInfo" :file-data="selectedFileData" />
    </el-dialog>
    <ConfirmationDialog
      :visible.sync="approveDialogVisible"
      :form.sync="approveForm"
      :request-type="selectedRow ? selectedRow.morespecific : ''"
...
export default {
  ...
  methods: {
    ...
    handleApprove() {
      const { message } = this.approveForm
      if (message) {
        // 更新状态和决定为'APPROVE'
